...
function Game(): ReactElement {
  ...
  const saveBattleMutation = useMutation({
    mutationFn: async (variables: {
      ...
      return await graphqlClient.request<{ saveBattleResult: MutationResponse }>(
        SAVE_BATTLE_RESULT,
        variables
      );
    },
  ...
  useEffect(() => {
    if (leftCard.data && rightCard.data && gameState === 'playing') {
      ...
      const battleWinner =
        newWinner === 'left' ? 'player' : newWinner === 'right' ? 'computer' : 'draw';
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      ...
      <Paper elevation={2} sx={{ p: 2, textAlign: 'center' }}>
        ...
        {isStatsLoading ? (
          <Box
            sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: 60 }}
          >
